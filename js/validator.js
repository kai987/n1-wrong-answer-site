...
export function normalizeImportItem(raw, index = 0) {
  ...
  return {
    ...
    context: text(raw.context, `${label}上下文/摘要`, { max: IMPORT_LIMITS.context }),
    passage: text(raw.passage ?? raw.readingPassage ?? raw.reading_passage ?? raw.article, `${label}读解文章`, { max: IMPORT_LIMITS.passage }),
    options,
    optionExplanations,
    userAnswer,
    correctAnswer,
